...
Testdata are created by activating profile 'testdata-(x)', with '(x)' being either 'bmw', 'hella', or 'osram'. Scripts are located at a respective folder below [db](src/main/resources/db). In order to clear all data from the database before, the profile 'testdata-truncate' needs to be activated additionally.
